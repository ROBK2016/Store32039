...
var sF$=(function(){
  ...
  function fnShowPrice(Price,OriginalPrice,Cod,iMaxParcels,ProductID){
      if(Price>=180)iMaxParcels=6;
      else if(Price>=150)iMaxParcels=5;
      else if(Price>=120)iMaxParcels=4;
      else if(Price>=90)iMaxParcels=3;
      else if(Price>=60)iMaxParcels=2;
  ...
  function fnLoginUserName(NameUser){
    ...
    else{jQuery('.loginInfo').html("<span class='col-small-6'>Olá, <b>visitante</b></span> <span class='hide-small'>|</span> <span class='col-small-6'>Faça seu <a href='/cadastro.asp?idloja="+FC$.IDLoja+"&pp=3&passo=1&sit=1'><b>Login</b></a></span>")}
  }
...
function fnMaxInstallmentsGrid(PrecoProd,MaxParcelas){
  ...
  if(typeof Juros!="undefined"){
    ...
    return "<br><span class=EstParc> em <b>"+MaxParcelas+"x</b>"+ComSem+" de <b class=EstParcPrice>"+FormatPrice(CalculaParcelaJurosCompostos(PrecoProd,MaxParcelas),FC$.Currency)+"</b></span>";
  }else{
    return "";
  }
}
...
function FuncChkRegisterBegin(){FCLib$.showPwdViewer();}
 //Função para o relogio cronometro regressivo
jQuery(document).ready(function() {
  if (jQuery("#DateCountdown1").length > 0){
    ...
    jQuery("#DateCountdown1").TimeCircles();
    jQuery("#DateCountdown2").TimeCircles();
    jQuery("#DateCountdown3").TimeCircles();
    ...
    jQuery(".startTimer").click(function() {
        jQuery("#CountDownTimer").TimeCircles().start();
    });
    ...
    jQuery(".fadeIn").click(function() {
        jQuery("#PageOpenTimer").fadeIn();
    });
    jQuery(".fadeOut").click(function() {
        jQuery("#PageOpenTimer").fadeOut();
    });
}
});


function InsertSeloPromo(iPrecoOri,iPrecoNum){
 if(iPrecoOri > iPrecoNum){ return document.write("<div class='XPTO'><img src='"+ FC$.PathImg +"selopromocao.png' /></div>")}
}
